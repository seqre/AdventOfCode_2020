...
import Data.HashMap.Strict (fromList, insert, findWithDefault, member)
...
main = do
    ...
    print ("Sol 2 improved: " ++ (show $ findMatching2' input))
...
findMatching2' :: [Int] -> Int
-- I belive it's 3SUM O(n^2)
findMatching2' [] = -2
findMatching2' x = let hashmap = fromList $ zip x x
                       len = length input
                       look e = findWithDefault 0 e hashmap
                    in  head [i * j * (2020 - s) | i <- x, j <- x, let s = i + j, member (2020 - s) hashmap]
